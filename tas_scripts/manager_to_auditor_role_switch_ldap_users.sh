...
# This will move ldap_users from manager to auditor and 
# add missing orginal org_manager users as space org_auditors so they do not use access
...
for org in $orgs; do
  ...
  for space in $spaces; do
    ...
    if [[ $(cat $Path_To_Config_Dir/config/$org_name/$space_name/spaceConfig.yml | yq -r '."space-manager".ldap_users | length') > 0 ]]; then
      ...
      for remove_dup_org_user in $remove_dup_org_users; do
        yq -i '."space-auditor".ldap_users += ['"\"$remove_dup_org_user\""']'  $Path_To_Config_Dir/config/$org_name/$space_name/spaceConfig.yml
      done
